...
It is noted that WaveICA can yield negative raw abundance values. When converting to a log2 scale, these values will become NA and may impact downstream analyses.
...
amide_wave$e_data[is.na(amide_wave$e_data)] <- NA
```
...
mix_waveica$e_data[is.na(mix_waveica$e_data)] <- NA
```
