...
Similar to SERRF, QC-RFSC requires complete observations, but unlike other methods, QC-RFSC must be ran on the abundance scale [@qcrfsc_paper]. As imputation is recommend on the log scale, the recommended procedure for QC-RLSC is to log transform the data, impute, transform back to raw abundance values and then run QC-RFSC. To compare results, it is suggested that users then convert data back to log2 scale after batch correction.

```{r}
amide_imp_abundance <- edata_transform(omicsData = amide_imp,data_scale = "abundance")
amide_qcrfsc_abundance <- bc_qcrfsc(omicsData = amide_imp_abundance,qc_cname = "group",qc_val = "QC",
...
```{r, out.width = "33%"}
p1 <- plot(dim_reduction(omicsData = pmart_amide),omicsData = pmart_amide,color_by = "batch") + labs(title = "Amide: Unadjusted")
